...
SELECT COUNT(*) AS Total_products_avialable
FROM zepto_v1;
...
SELECT name, discountedSellingPrice * availableQuantity AS total_revenue
FROM zepto_v1;
...
SELECT name, (discountedSellingPrice * 1.0) / weightInGms AS price_per_gram
FROM zepto_v1
WHERE weightInGms > 0
ORDER BY price_per_gram;

-- 12. Find best value products based on lowest price per gram.
SELECT name, (discountedSellingPrice * 1.0) / weightInGms
 AS price_per_gram
FROM zepto_v1
WHERE weightInGms > 0
